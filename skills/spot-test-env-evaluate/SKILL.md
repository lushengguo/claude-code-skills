---
name: spot-test-env-evaluate
description: 启动 KCEX Spot 连接测试环境，观察做市行为和订单管理，检查限流/铺单/模式切换是否正常
---

# Spot Test Env Evaluate

启动 KCEX Spot C++ 做市系统连接测试环境（`test-env.yaml`），运行几分钟，
...
# 2. 启动（模拟 launch.json 的 "Spot (test-env-debug)"）
cd /kcex/cpp/spot
DEPLOY_PROCESS=E-1-1 KMM_RUNTIME_ENV=test LOG_LEVEL=INFO LOG_HISTORY=1 \
./build-debug/kcex-mm-spot \
  --config dev/config/test-env.yaml \
  --loop \
  --interval-ms 3000 \
  > /tmp/spot_test_run.log 2>&1 &
```
...
| 变量 | 值 | 作用 |
|---|---|---|
| `DEPLOY_PROCESS` | `E-1-1` | 部署分组，映射到 MySQL `spot_symbol_configs` 表的 `process_group` 列 |
| `KMM_RUNTIME_ENV` | `test` | 运行时环境标识 |
| `LOG_LEVEL` | `INFO` | 日志级别（INFO 可看到 tick_done；DEBUG 有更详细的 step_tick/fallback 日志） |
| `LOG_HISTORY` | `1` | 启用 log history（`/loglevel` admin endpoint 可用） |

### 测试环境依赖

`dev/config/test-env.yaml` 连接到真实测试环境：
- Exchange API: `http://openapi.kcextest.com/spot`
- MySQL: test RDS (`kcex-test-maker-common.cluster-cpyw088uimn5.ap-northeast-1.rds.amazonaws.com`)
- Redis: test cluster (`test-common-redis-cluster.zjhkak.clustercfg.apne1.cache.amazonaws.com`)
- `SUBMIT_ENABLED: true` — 会实际向测试交易所下单
...
```bash
# 等待进程启动并加载 symbol
until grep -q "tick_done" /tmp/spot_test_run.log 2>/dev/null; do sleep 2; done
```

### 2. tick_done 汇总日志（每个 tick 一行）
```bash
grep "tick_done" /tmp/spot_test_run.log | tail -10
```
...
### 3. 模式切换
```bash
grep "mode_transition\|price_ban_mode\|price_default_grid_mode" /tmp/spot_test_run.log | tail -10
```

### 4. Reconcile 行为（每 20 tick 一次）
```bash
grep "reconcile\|openOrders" /tmp/spot_test_run.log | tail -10
```

### 5. 错误和限流
```bash
grep "ERROR\|WARNING\|rate_limit" /tmp/spot_test_run.log | tail -20
```

### 6. 停止进程
```bash
pkill -f "kcex-mm-spot.*test-env"
```
...
grep "tick_done" /tmp/spot_test_run.log | \
  awk '{print $0}' | \
...
#### 7b. 首次 tick（冷启动盘口）
```bash
# 第一个 tick_done 显示初始盘口状态——通常 planned add > 0, executed 取决于是否被限流
grep "tick_done" /tmp/spot_test_run.log | head -3
```

#### 7c. 最后几个 tick（稳态盘口）
```bash
# 稳态下 planned cxl/add 应趋近于 0（盘口与目标一致，无需变更）
grep "tick_done" /tmp/spot_test_run.log | tail -5
```

#### 7d. 限流时的盘口更新（重点）
```bash
# 限流 tick：planned 有值但 executed 全为 0，盘口未实际更新
grep "tick_done" /tmp/spot_test_run.log | grep -v "rate_limit=0" | head -10

# 限流恢复后的第一个成功 tick：通常 executed add 较大（积压的订单一次性提交）
grep "tick_done" /tmp/spot_test_run.log | grep "rate_limit=0" | grep -v "executed(cxl=0 add=0"
```

#### 7e. Grid 网格更新日志
```bash
# Emergency 网格（default_grid）：仅在 Emergency 模式触发
grep "default_grid_orders_replace" /tmp/spot_test_run.log

# Ban 网格：仅在 Ban 模式触发
grep "ban_orders_replace" /tmp/spot_test_run.log
```
...
grep "plan_update_done" /tmp/spot_test_run.log | tail -20
```
...
#### 7g. Reconcile 对盘口的影响
```bash
# 冷启动对账：发现交易所残留订单，取消 unknown，种子化 known
grep "reconcile_cold_start" /tmp/spot_test_run.log

# 稳态对账：清理过期/膨胀订单
grep "reconcile_oversized\|reconcile_expired" /tmp/spot_test_run.log

# 未知订单取消（可能是上次运行残留）
grep "reconcile_cancel_unknown" /tmp/spot_test_run.log
```

#### 7h. 网格订单详情（盘口价格和数量）
```bash
# special_grid_done 显示 Emergency/Ban 网格的具体价格
# firstAsk= / firstBid= 是网格最内侧价格（盘口的"宽度"参考）
grep "special_grid_done" /tmp/spot_test_run.log | tail -5
```

#### 7i. 限流恢复追踪
```bash
# 限流退避清除（consecutiveRateLimits_ 归零）
grep "429_backoff_cleared" /tmp/spot_test_run.log
```

#### 7j. 交易所批次详情（需 DEBUG 级别日志）
```bash
# cancel_add_done 显示每次 cancelAndAdd 批次的接受情况
grep "cancel_add_done" /tmp/spot_test_run.log | tail -20
```
...
- [ ] `[cancel_batch_failed]` 是 reconcile 路径的预期行为（测试环境交易所 400 表示订单已成交/取消）
...
`ban_update` 在 exec layer 设于 `submitBanGrid` 成功后。如果 exchange 返回 429，`submitBanGrid` 提前返回，`ban_update` 不设。这是正确的——表示 ban 网格计划了但未实际提交。

### 测试环境 429 频繁
测试交易所限流激进（可能每 symbol 每 batch 都触发）。这不影响做市逻辑正确性——计划层仍正常产出，只是执行层被限流拦截。
